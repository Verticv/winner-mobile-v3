  ...
  const HistoryMenu = ({
    ...
    function TabsList({ items }) {
      return items.map((item, index) => (
        <button
          className={`${
            pathname === item.path ? 'tabslistspeedkino' : 'tabslistspeedkino1'
          } tabslistspeedkino2`}
          style={{
            height: '6.375rem',
            fontSize: '2.8125rem',
            marginRight: `${!index ? '0.9375rem' : ''}`,
    ...
    return items.map((item, index) => (
      ...
      >
        <div className="content-cells">
          {item.month} [{item.roundNumber}]
        </div>
        <div
          className="content-cells"
          style={{
            paddingLeft: '0.3rem',
            paddingTop: '0.2rem',
          }}
            ...
            {item.departureNumber && (
              <div
                style={{
                  ...
                  top: '-0.5rem',
                  right:
                    item.departureNumberDirection === 'right' ? '-0.75rem' : '',
                  left:
                    item.departureNumberDirection === 'left' ? '-0.75rem' : '',
                }}
                className={`content-cells3`}
              >
                {item.departureNumber}
              </div>
            )}
              ...
              <div className="speedkino-sum">
                <div style={{ fontSize: '2.25rem' }} className="speedkino-sum1">
                  <div
                    ...
                    className="speedkino-sum2"
                  >
                    회차
                  </div>
                  <div style={{ height: '5.25rem' }} className="speedkino-sum2">
                    언오버
                  </div>
                </div>
              </div>
