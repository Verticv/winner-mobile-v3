...
import HorizontalSubMenu from '../../components/MyPage/HorizontalSubMenu';
...
const Faq = ({ subActiveButton, setSubActiveButton }) => {
  const tabsArray2 = [
    ...
    },
    {
      ...
      id: 5,
      width: '18.3125rem',
      path: '/cscenter/all/faq/hotelcasino',
    },
    {
      text: '홀덤',
      id: 6,
      width: '11.5625rem',
      path: '/cscenter/all/faq/holdom',
    },
    {
      text: 'e-스포츠',
      id: 7,
  ...
  const questionArray = [
    {
      id: 0,
      type: '충전/환전',
      text: '첫, 매충 10% 포인트를 받으면 롤링을 어떻게 해야 환전가능한가요?',
    },
    {
      id: 1,
      type: '충전/환전',
      text: '1,000원 단위도 충전/환전 신청이 가능한가요?',
    },
    {
      id: 2,
      type: '충전/환전',
      text: '충전시 은행 입금자명이 달라도 처리가 가능한가요?',
    },
    {
      id: 3,
      type: '충전/환전',
      text: '출금(환전)시 예금주 변경 및 타명의 사용이 가능 &nbsp; &nbsp; 한가요?',
    },
    {
      id: 4,
      type: '베팅관련',
      text: '최소 베팅금액, 최대 베팅금액, 최대 적중한도 금액은 얼마인가요?',
    },
    {
      id: 5,
      type: '충전/환전',
      text: '충전&환전 처리 시간은 어느 정도 소요되나요?',
    },
    { id: 6, type: '베팅관련', text: '베팅 취소는 가능한가요?' },
    {
      id: 7,
      type: '스포츠',
      text: '스포츠북 라이브 베팅 목록 중, 경기 도중에 베팅한 &nbsp; 경기가 사라지는 경우 베팅한 내역은 어떻게 처리 &nbsp; &nbsp; 되나요?',
    },
    {
      id: 8,
      type: '기타',
      text: 'Winner 홈페이지에 접속이 잘 되지 않거나 사용도중 에러가 발생하는데 어떻게 해야 하나요?',
    },
    { id: 9, type: '기타', text: '이 사이트는 안전한가요?' },
    {
      id: 10,
      type: '기타',
      text: '중복 IP 또는 본인 아이디를 공유하여 사용이 가능 &nbsp; 한가요?',
    },
  ];

  const BodyText1 =
    '☞ 첫, 매충 10% 포인트를 받지 않을시 모든게임의 롤링은 단폴, &nbsp; &nbsp; 카지노 베팅 포함 100%만 해주시면 환전가능합니다.';
  const BodyText2 =
    '- 포인트 받지 않기를 원할시 : \n 충전 --> "보너스받기" 에서 "받지않기" 클릭하시고 신청해주시면 됩니다.';
  const BodyText3 =
    ' ☞ 첫, 매충 10%포인트를 받았을시 각게임당 아래와 같이 롤링해  &nbsp; 주시면 됩니다. ';
  const BodyText4 =
    '언제나 고객에게 최상의 서비스 제공을 할 수 있도록 노력하는 &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; \n  &nbsp;『 Winner 』가 되겠습니다.';
  ...
  const QuestionCell = ({ type, text, cellId }) => (
    <>
      ...
      >
        <img className="q-icon" src={QIcon} alt="" />
        <div
          style={{ marginLeft: '1.75rem' }}
          className="w-full h-full flex-col flex flex-wrap justify-center items-start"
        >
          <div
            className="q-type"
            style={{
        ...
        <img
          ...
          className={`${
            openedCell === cellId && 'transform rotate-180'
          } q-down-arrow`}
          src={openedCell === cellId ? arrowUp : ArrowDown}
          alt=""
        />
      </button>
      {openedCell === cellId && (
