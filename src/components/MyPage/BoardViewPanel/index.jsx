    ...
    return (
        <>
            {attachedArray && attachedArray?.map((card, index) => (
                <div key={card.id} style={{ margin: '1.5rem', background: 'linear-gradient(to top,#ecebeb, #dbb0e8 50%, #cb78e6 )', boxShadow: '0 0 1rem 0 rgba(0, 0, 0, 0.3)', padding: '0.1875rem', borderRadius: '1rem'}}>
                    <div style={{ borderRadius: "1rem", paddingBottom: '0.5625rem', color: '#ccc2b6', background: '#ffffff' }} className="shadow-subNavbar bg-gray-fafafa">
                        <TableHeader smallCard />
                        <div style={{ padding: '0 1rem' }} className="w-full">
                            <div className="w-full rounded-lg" style={{ padding: '0.2875rem 0.1875rem', marginTop: '1rem' }}>
                                <div className='rounded-lg w-full shadow-box-myPage' style={{ background: '', border: '0.45rem solid rgb(104, 82, 167)', borderRadius: '0.8rem' }}>
                                    <CardHeader flag={card.flag} name={card.name} date='06-29 15:45' />
                                    <CardResults
                                        bet={card.bet}
                                        result={card.result}
                                        score={card.score}
