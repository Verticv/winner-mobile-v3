...
const HorizontalSubMenu1 = ({
  ...
  function TabsList({ items }) {
    return items.map((item, index) => (
      <button
        className={`${
          pathname === item.path ? 'text-gray-r454545' : 'text-gray-b7b7b7'
        } rounded-full min-w-fit text-black flex items-center`}
        style={{
          height: '6.375rem',
          minWidth: 'fit-content',
          fontSize: '2.625rem',
          marginRight: `${!index ? '0.9375rem' : '0.9375rem'}`,
          background: 'bottom',
          color: pathname === item.path ? '#ffdfbd' : '#929292',
          display: 'flex',
          alignItems: 'center',
          fontWeight: '700'
        }}
      ...
      >
        <div style={{ margin: '0px 3.8rem 0 2.8rem' }}>
          <div>
            <span
              style={{
                color: `${pathname === item.path ? '#ffffff' : '#cfa2ec'} `,
              }}
            >
              {item.text}
            </span>
