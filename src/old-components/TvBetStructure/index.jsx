...
const TvBetStructure = () => {
  ...
  const TitleText = ({ number = "01", titleText = "화면구성", noNumber }) => (
    <div className="flex items-center" style={{ display: 'flex', alignItems: 'center', fontSize: '3.41188rem', paddingTop: '2rem', paddingBottom: '1.8rem' }}>
      ...
      <span className="text-22px tracking-tight text-gray-r585858 ml-3px h-22px flex items-center" style={{ letterSpacing: '-0.07rem', marginTop: '0.1rem', marginRight: '1rem', color: '#ccc2b6' }}>{titleText}</span>
      <div style={{ backgroundColor: "#494745", borderRadius: "100%", padding: '0.4rem 0.5rem', color: '#ccc2b6', visibility: noNumber ? "hidden" : "visible" }} className="w-36px h-36px ml-3px flex items-center justify-center text-white text-22px font-roboto">
        {number}
      </div>
    </div>
  )
  ...
  const Item = ({ number = "1.", title = "게임종류선택", text = "다양한 게임을 선택할 수 있습니다.", marginTop, text2 = null, text3 = null, text4 = null, color1 = null, color2 = null, color3 = null, isflex, withoutP }) => (
    <div className="flex flex-col space-y-4px" style={{  lineHeight: '1.4', marginTop: marginTop }}>
      ...
      <p style={{ color: "#828282", fontSize: '2.625rem', margin: 0 }} className="flex h-16px items-center text-16px tracking-tight ">{text} <span className="ml-2px text-blue-r0056a6" style={{ color: '#4c98ff', marginLeft: '0.5rem', marginRight: '0.2rem' }}>{color1}</span><span style={{ color: "#e65454", marginRight: '0.5rem' }} className="mr-2px">{color2}</span><span style={{ margin: isflex || withoutP ? '0' : '' }}>{color3}</span></p>
      {text2 && (
        <p style={{ color: "#8c8c8c", margin: 0, fontSize: '2.625rem' }} className="flex h-16px items-center text-16px tracking-tight ">{text2}</p>
      )}
      {text3 && (
        <p style={{ color: "#8c8c8c", margin: 0, fontSize: '2.625rem' }} className="flex h-16px items-center text-16px tracking-tight ">{text3}</p>
      )}
      {text4 && (
        <p style={{ color: "#8c8c8c", margin: 0, fontSize: '2.625rem' }} className="flex h-16px items-center text-16px tracking-tight ">{text4}</p>
      )}
    </div>
  )
  ...
  const LongItem = ({ number = "4.", title = "메뉴", marginTop, text = "결과 : 경기결과 확인 가능", text2 = "팔레이 : 팔레이(다폴더) 리스트 확인 가능", text3 = "내베팅 : 상세한 베팅내역 확인 가능  ", text4 = "베팅규정 : 베팅규정 확인 가능", text5 }) => (
    <div className="flex flex-col space-y-4px" style={{  lineHeight: '1.4', marginTop: marginTop, fontSize: '2.625rem' }}>
      <div className="flex text-18px tracking-tight space-x-2px flex h-18px items-center" style={{  fontSize: '2.825rem' }}>
        <span className="text-blue-r0056a6" style={{ color: '#a67c52', marginRight: '0.4rem' }}>{number}</span>
        <span className="text-gray-r585858" style={{ color: '#ccc2b6' }}>{title}</span>
      </div>
      ...
      <div className="flex" style={{ marginTop: '-0rem' }}>
        <p style={{ color: "#8c8c8c", margin: 0 }} className="flex h-16px items-center text-16px tracking-tight ">• {text4}</p>
      </div>
      {text5 && (
        <div className="flex" style={{ marginTop: '-0rem' }}>
          <p style={{ color: "#8c8c8c", margin: 0 }} className="flex h-16px items-center text-16px tracking-tight ">• {text5}</p>
        </div>
      )}
    </div>
  )
