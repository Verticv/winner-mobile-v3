    ...
    <div className="container relative">
      {!gameMarket && !isOpenInfo && (
        <>
          ...
          {activeButton === 'menu' && (
            <CategorySections close={() => setActiveButton(null)} />
          )}
          {activeButton === 'favorite' && (
            <FavoritesSection close={() => setActiveButton(null)} />
          )}
          {activeButton === 'win' && (
            <Section3 close={() => setActiveButton(null)} />
          )}
          {activeButton === 'win2' && (
            <Section4 close={() => setActiveButton(null)} />
          )}
          {activeButton === 'search' && (
            <SearchSection close={() => setActiveButton(null)} />
          )}
        </>
      )}
      {gameMarket && !isOpenInfo && (
        <GameMarkets close={() => showGameMarket(false)} />
      )}
